...
@Component({
	selector:'marketing-consents-save-popup',
    templateUrl: './marketing.consents.save.popup.component.html',
})


export class MarketingConsentsComponentSavePopup implements OnChanges {
	@Input() saveConsentsArray= [];
	@Input() saveSelectedAll:boolean = false;
	@Input() savePage:string;
	@Input() page:string;
	@Input() nextClicked:boolean;	
	@Output() onDataPickedSavePopup: EventEmitter<any> = new EventEmitter<any>();
	...
	ngOnChanges() {
		this.consentsArr = this.saveConsentsArray;
		this.selectedAllConsent = this.saveSelectedAll;
		this.pageId = this.savePage;
	...
	sendData(){
		...
		let obj = {"IsConsentValid":this.checkMarketingMandatory(), "ConsentArray":this.consentsArr, "SelectedAllConsents":this.selectedAllConsent};
		if(this.pageId == "SavePopup"){
			this.onDataPickedSavePopup.emit(obj); 
		}
